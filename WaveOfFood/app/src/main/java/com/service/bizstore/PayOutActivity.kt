...
import com.service.bizstore.Fragment.CongratsBottomSheet
import com.service.bizstore.databinding.ActivityPayOutBinding
import com.service.bizstore.Model.OrderDetails
...
class PayOutActivity : AppCompatActivity() {
    ...
    private fun calculateTotalAmount(): Int {
        ...
        for (i in 0 until foodItemPrice.size) {
            ...
            if (!price.matches(Regex("\\d+\\$?"))) {
                Toast.makeText(this, "Invalid price format: $price", Toast.LENGTH_SHORT).show()
                continue
            }

            val priceIntValue = try {
                val cleanPrice = if (price.lastOrNull() == '$') {
